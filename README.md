...
- This is a living document, it's still being updated from time to time. The document is getting long so I'll have to cut some of these to another file.

## [Author](https://github.com/Huangphoux/) Notes

-   This is a distilled version of my personal notes from reading various thinkpieces about one of the best ways of making websites / web apps when using Datastar.
...
-   If you're comfortable with and productive using React or [htmx](https://htmx.org/), keep using it. Changing tools is not only a technical decision; it's also [sociological](https://htmx.org/essays/when-to-use-hypermedia/#if-your-team-is-not-on-board).
    -   Rethink twice before using React. Consider reading some articles compiled on this [website](https://jsx.lol/).
-   The shift in thinking from htmx to Datastar, is as big as from React to htmx.

# Datastar

-   Use Datastar to build [real-time](https://example.andersmurphy.com/), [collaborative](https://checkboxes.andersmurphy.com/) web apps without losing any [performance](https://andersmurphy.com/2025/04/07/clojure-realtime-collaborative-web-apps-without-clojurescript.html).
-   You can also use it to build simple websites as well, since it's a [simpler mental model](https://yagni.club/3m475dwkjvc2o/l-quote/39_0-39_329#39_0).
-   Here are some of the [alternatives](https://htmx.org/essays/alternatives/) suggested by [Carson Gross](https://bigsky.software/cv/) and a [comparision](https://nomini.js.org/comparison/) between htmx, [Nomini](https://nomini.js.org/), Alpine, and Datastar.
-   The idea of which to use isn't important, LOL; just send HTML over the wire and call it a day.
...
-   Morphlex, Datastar's TS port

## CQRS

-   If we send the entire `<body>`, we should limit the number of endpoints that can change the view
-   Command Query Responsibility Segregation: separate Commands and Queries
-   Commands = `Create`, `Update` and `Delete`
    -   `POST`, `PUT` or `PATCH`, `DELETE`: change the data
...
-   By using CQRS, our app become the purest `view=function(state)`
-   A function turning state into view: present data using HTML, then compute it as a page
